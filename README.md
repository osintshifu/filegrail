...
| **Maker notes** | `.jpg` `.jpeg` `.jpe` `.tif` `.tiff` `.dng` `.nef` `.cr2` `.arw` `.orf` `.rw2` `.webp` `.heic` `.heif` `.avif` | The vendor block beside the EXIF: camera body serial number, shutter count, the owner name typed into the camera, firmware version, frame number and lens identity for Canon and Nikon, and for Apple the image and content identifiers that tie a Live Photo's still and film together. Apple, Olympus, Panasonic, Fujifilm and Konica Minolta blocks are located and their entries counted; a block this reader cannot place is reported as present rather than guessed at. A note holding a picture instead of a directory is read as a preview |
