...
| `maker-notes` | `.jpg` `.jpeg` `.jpe` `.tif` `.tiff` `.dng` `.nef` `.cr2` `.arw` `.orf` `.rw2` `.webp` `.heic` `.heif` `.avif` | Camera body serial number, shutter count, owner name, firmware version, frame number and lens identity from Canon and Nikon blocks; Apple image and content identifiers; a preview image where the block holds one; and for every vendor the name, the entry count, how the offsets are addressed and whether the byte order matches the container |
